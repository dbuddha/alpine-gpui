...
and shutdown-cancel actions remain in the behavior. A running job remains owned
...
distinct terminal actions and current or stale occurrence witnesses preserve
every modeled outcome consumed by the checked properties. Terminal jobs clear
their revision tags after no later transition or checked property can consume
them. `TerminalJobsHaveNoTag` enforces that quotient explicitly. A direct
`applicationIsCurrent` safety monitor remains true for the exact-tagged apply
action and becomes false only for the deliberate stale-apply control; it avoids
retaining historical disposition identity in unrelated states.
