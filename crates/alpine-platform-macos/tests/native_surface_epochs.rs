...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dirty = surface.snapshot();
    assert!(!dirty.display_link_paused());

    native_validation::inject_surface_configuration(&surface, 0.0, 64.0, 1.0, 101, true)?;
    let zero = surface.snapshot();
    assert_eq!(zero.surface_epoch(), dirty.surface_epoch() + 1);
    assert!(!zero.is_sized());
    assert!(zero.display_link_paused());
    assert_eq!(zero.submission_count(), dirty.submission_count());
    assert_eq!(zero.direct_present_count(), dirty.direct_present_count());
    assert_eq!(zero.allocated_bytes(), dirty.allocated_bytes());
    assert_eq!(
        zero.current_retained_bytes(),
        dirty.current_retained_bytes()
    );
    ...
    let invalid =
        native_validation::inject_surface_configuration(&surface, 16_385.0, 64.0, 1.0, 101, true);
    assert!(matches!(
        invalid,
