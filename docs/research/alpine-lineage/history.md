...
| 2026-09-02 | Experiment #553 compared command-buffer-owned presentation with the current direct-present path | Exact executable hashes, isolated session state, identical 55-byte input, normalized profile-v2 streams, deterministic analyses, visual and byte equivalence, memory observations, and clean close are retained under `assurance/studio-profile/v2/553-command-buffer-presentation` | Rejected: both runs retained the exact 33,333,250 ns display-target-to-presentation-target offset, and the 0.810 ms actual-presentation shift was already present at the display-link target; display-link scheduling and bounded high-rate input-tail experiments remain #304/#331 |
