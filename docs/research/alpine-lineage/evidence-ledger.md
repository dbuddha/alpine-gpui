...
| ALG-014 | CPU oracle and versioned trace | Comparator methodology, `ALPINE-ORIGINAL`, E2 | Immutable v1 solid-quad control, v2 prepared scenes, and a separate bounded lifecycle companion for atlas reuse, replacement, teardown, and full resynchronization | PR #343, Alpine Zed Lab PR #5, exact `assurance/lab/v2` evidence, plus Task #353 sequence property, Kani, CLI, and CPU-oracle evidence | Eight prepared scenes are composed E3; lifecycle protocol is implemented in Alpine but physical Direct Metal and pinned GPUI sequence evidence remain before lifecycle E3, timing, memory, and E4 |
