...
`alpine-editor` owns the tree, cache, ignore matcher stack, and row projection. It adds no dependency, public API, native handle, worker, watcher, timer, polling loop, or async runtime. The existing `ignore` dependency parses project-local rules, but each request uses immediate `read_dir` enumeration rather than a recursive walker. A bounded max-heap makes cap behavior independent of filesystem enumeration order by retaining shortest portable paths first, then entry kind and bytewise path for ties; accepted rows are displayed directory-first and bytewise. Result publication accounts retained paths and entry counts before mutation. Prefix rows are recomputed only after accepted tree changes; paint uses subtree spans to skip non-visible ranges, shares each row path with a compact name offset, and shapes only the bounded returned rows without allocating a second label string.
