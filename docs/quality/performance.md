...
hardware. Opt-in Kani proofs may establish structural properties such as bounded
...
perform one separately disclosed semantic admission render, discard the
declared warmups, require every image to match the admitted preflight image, and
atomically publish strict `sample_index,elapsed_ns` CSV without replacement.
The measured stage is explicitly `renderer-submit-readback` on a
