...
impl TestWorkspace {
    fn new() -> std::io::Result<Self> {
        let sequence = NEXT_TEST_FILE.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!(
            "alpine-editor-workspace-{}-{sequence}",
            std::process::id()
        ));
        fs::create_dir(&path)?;
        Ok(Self { path })
...
fn test_app() -> Result<EditorApp, SurfaceError> {
    EditorApp::new(TestTextSystem)
}
...
-> Result<(), Box<dyn Error>> {
    let mut app = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("head"), None)?;
    ...
    assert_eq!(
        EditorApp::caret_scroll_target(expected, line_top, content_height, app.maximum_scroll())
            .to_bits(),
        expected.to_bits()
    );
    assert_eq!(
        EditorApp::caret_scroll_target(
            app.maximum_scroll(),
            line_top,
            content_height,
            app.maximum_scroll(),
    ...
    assert!(!EditorApp::selection_changed(app.selection, app.selection));
    assert!(EditorApp::selection_changed(
        Selection::caret(ByteOffset::new(0)),
        app.selection,
    ));

    let mut invalid_layout =
        EditorApp::from_document(TestTextSystem, EditorDocument::scratch("head"), None)?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let mut app = EditorApp::new(CountingTextSystem {
...
fn release_profile_event_kind_vocabulary_is_stable() -> Result<(), Box<dyn Error>> {
    assert!(!EditorProfiler::disabled().enabled());
    EditorProfiler::default().record(EditorSignpost::new(
...
-> Result<(), Box<dyn Error>> {
    ...
    let (profiler, records) = EditorProfiler::recording();
    ...
    let stages: Vec<EditorSignpostStage> =
        records.iter().copied().map(EditorSignpost::stage).collect();
    let position = |stage| stages.iter().position(|candidate| *candidate == stage);
    let event_begin = position(EditorSignpostStage::EventDispatchBegin).ok_or("event begin")?;
    let mutation = position(EditorSignpostStage::StateMutationComplete).ok_or("mutation")?;
    let frame_begin = position(EditorSignpostStage::FrameBuildBegin).ok_or("frame begin")?;
    let layout_begin = position(EditorSignpostStage::VisibleLayoutBegin).ok_or("layout begin")?;
    let layout_end = position(EditorSignpostStage::VisibleLayoutComplete).ok_or("layout end")?;
    let atlas_begin = position(EditorSignpostStage::AtlasPublicationBegin).ok_or("atlas begin")?;
    let atlas_end = position(EditorSignpostStage::AtlasPublicationComplete).ok_or("atlas end")?;
    let text = position(EditorSignpostStage::TextSummary).ok_or("text summary")?;
    let frame_end = position(EditorSignpostStage::FrameBuildComplete).ok_or("frame end")?;
    ...
    let text_record = records
        .iter()
        .find(|record| record.stage() == EditorSignpostStage::TextSummary)
        .ok_or("text record")?;
    assert!(text_record.values()[0] > 0);
    assert!(text_record.values()[1] > 0);
    drop(records);

    let mut failing = EditorApp::new(FailingTextSystem)?;
    let (profiler, failed_records) = EditorProfiler::recording();
    ...
    assert!(matches!(
        ...
        Err(EditorRenderError::Layout(LayoutError::ArithmeticOverflow))
    ));
    ...
    assert!(
        failed_records
            .iter()
            .any(|record| record.stage() == EditorSignpostStage::AtlasPublicationFailed)
    );
    let failed = failed_records
        .iter()
        .position(|record| record.stage() == EditorSignpostStage::FrameBuildFailed)
        .ok_or("failed frame")?;
    let completed = failed_records
        .iter()
        .position(|record| {
            record.stage() == EditorSignpostStage::FrameBuildComplete && record.values()[2] == 1
        })
...
fn glyph_geometry_and_atlas_publication_axes_are_exact() -> Result<(), Box<dyn Error>> {
    let mut app = EditorApp::new(GeometryTextSystem)?;
    let viewport = viewport()?;
...
fn full_atlas_adaptation_rejects_malformed_bytes() -> Result<(), Box<dyn Error>> {
    ...
    assert!(EditorApp::full_atlas_image(1, two, vec![255_u8].into_boxed_slice()).is_err());
    Ok(())
}
...
fn assert_focus_epoch_cancels_owner(app: &mut EditorApp) -> Result<(), EditorRenderError> {
    ...
    let future_epoch = next_epoch.checked_next().ok_or(EditorRenderError::Domain)?;
    assert!(
...
-> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert!(app.handle_event(&ime(ImeEvent::Started)).visual_changed);
...
-> Result<(), EditorRenderError> {
    let mut editor = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert_focus_epoch_cancels_owner(&mut editor)?;

    let mut find = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert!(find.find.open(false));
    assert_focus_epoch_cancels_owner(&mut find)?;

    let mut quick_open = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert!(
        quick_open
            .quick_open
            .open(1)
    ...
    let mut command_palette = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert!(command_palette.open_command_palette().visual_changed);
    assert_focus_epoch_cancels_owner(&mut command_palette)?;

    let mut project_search = test_app().map_err(|_| EditorRenderError::Domain)?;
    assert!(
        project_search
            .project_search
            .open(1)
...
fn clipboard_policy_controls_distinguish_each_response_boundary() -> Result<(), SurfaceError> {
    ...
    assert_eq!(
        editor_clipboard_shortcut(&shortcut(Modifiers::COMMAND)),
    ...
    for modifiers in [
        ...
        assert_eq!(editor_clipboard_shortcut(&shortcut(modifiers)), None);
    }
...
#[test]
fn editor_scene_contains_clipped_glyphs_atlas_and_caret() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
...
#[test]
fn editor_scene_projects_compiled_rust_syntax_onto_visible_glyphs() -> Result<(), EditorRenderError>
{
    let document = EditorDocument::scratch("pub fn Main() { let count = 42; // local\n}");
    let mut app = EditorApp::from_document(TestTextSystem, document, Some(Path::new("main.rs")))
        .map_err(|_| EditorRenderError::Domain)?;
    let scene = app.try_scene(
        SceneRevision::new(1),
        viewport().map_err(|_| EditorRenderError::Domain)?,
    ...
    let render_error = EditorRenderError::from(SyntaxError::InvalidBudget);
    assert_eq!(
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = EditorApp::open_file(TestTextSystem, root.path().join("main.rs"))?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    *app.buffer_mut() = Buffer::new("alpha beta alpha");
    ...
    let mut tab_app = EditorApp::open_workspace(TestTextSystem, root.path())?;
...
#[test]
fn status_raster_failure_is_structured_after_empty_document_layout() -> Result<(), EditorRenderError>
{
    let mut app = EditorApp::new(FailingRasterTextSystem).map_err(|_| EditorRenderError::Domain)?;
    *app.buffer_mut() = Buffer::new("");
    ...
    assert!(matches!(
        ...
        Err(EditorRenderError::Layout(LayoutError::NativeFailure(
            "injected status raster failure"
        )))
    ));

    let mut overlay =
        EditorApp::new(FailingRasterTextSystem).map_err(|_| EditorRenderError::Domain)?;
...
#[test]
fn editor_runtime_returns_clipboard_and_dirty_close_responses()
-> Result<(), Box<dyn std::error::Error>> {
    let viewport = viewport()?;
    let clear = LinearRgba::new(0.02, 0.02, 0.02, 1.0).ok_or(SurfaceError::invariant(
        alpine_platform_macos::SurfaceOperation::Application,
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    root.write("alpha.rs", "alpha")?;
    root.write("beta.rs", "beta")?;
    let mut activation_error = EditorApp::open_workspace(TestTextSystem, root.path())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        EditorApp::open_file(TestTextSystem, invalid.path()),
        Err(EditorError::File(FileError::InvalidUtf8))
    ));
    ...
    let usage = EditorError::Usage;
    assert_eq!(usage.to_string(), "usage: alpine-editor [path]");
    assert!(usage.source().is_none());
    let file_error = EditorError::from(FileError::InvalidUtf8);
    assert!(file_error.to_string().contains("Studio file failed"));
    assert!(file_error.source().is_some());
    let runtime_error = EditorError::from(RuntimeError::Surface(SurfaceError::UnsupportedPlatform));
    assert!(runtime_error.to_string().contains("Studio runtime failed"));
    assert!(runtime_error.source().is_some());
    let recovery_error = EditorError::Recovery {
...
fn dirty_recovery_fixture(
    ...
    let mut app = EditorApp::open_file(TestTextSystem, document_path)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut clean_app = EditorApp::open_file(TestTextSystem, &recovered_path)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        ...
        Err(EditorError::Workspace(WorkspaceError::UnsupportedTarget(path)))
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let truncated_app = EditorApp::from_workspace(TestTextSystem, workspace)?;
    assert_eq!(
        truncated_app
            .local_status
            .as_ref()
    ...
    let mut app = EditorApp::open_workspace(TestTextSystem, rendered_root.path())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let missing = std::env::temp_dir().join("alpine-editor-definitely-missing-workspace");
    ...
    let wrapped = EditorError::from(WorkspaceError::AllocationFailed);
    ...
    let quick_open_render = EditorRenderError::from(QuickOpenError::InvalidLimits);
    assert!(
        quick_open_render
            .to_string()
    ...
    let file_error = EditorDocument::open(invalid_file.path())
        .err()
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        EditorApp::validate_navigation_document(&unavailable),
        Err(WorkspaceSelectionError::File(FileError::Conflict(
            ExternalChange::Deleted
        )))
    ));

    let mut scratch_app = EditorApp::new(TestTextSystem)?;
    assert!(matches!(
...
fn workspace_scene_geometry_and_scroll_routing_are_exact() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let mut pointer_app = EditorApp::open_workspace(TestTextSystem, root.path())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let text = "line\n".repeat(100);
    let mut app = EditorApp::from_document(TestTextSystem, EditorDocument::scratch(&text), None)?;
    let viewport = viewport()?;
    ...
    assert!(
        app.dispatch_command(EditorCommand::SplitRight)
            .visual_changed
    );
    ...
    assert!(
        app.dispatch_command(EditorCommand::SplitDown)
            .visual_changed
    );
    assert_eq!(app.panes.len(), 3);
    assert!(
        app.dispatch_command(EditorCommand::FocusNextPane)
            .visual_changed
    );
    assert!(
        app.dispatch_command(EditorCommand::ClosePane)
            .visual_changed
    );
    assert_eq!(app.panes.len(), 2);
    Ok(())
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut single = EditorApp::from_document(TestTextSystem, EditorDocument::scratch(text), None)?;
...
fn pane_document_failure_boundaries_fail_closed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    root.write("alpha.rs", "alpha")?;
    root.write("beta.rs", "beta")?;
    let mut invalid_switch = EditorApp::open_workspace(TestTextSystem, root.path())?;
    ...
    invalid_switch
        .tabs
        .inject_active_payload_for_test(EditorDocument::scratch("duplicate active payload"));
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let click = |index: usize, timestamp: u64| -> Result<SurfaceEvent, EditorRenderError> {
...
fn tab_fault_paths_are_structured_and_non_destructive() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.tabs
        .inject_active_payload_for_test(EditorDocument::Scratch {
...
#[test]
fn ime_preview_is_non_destructive_and_commit_is_atomic() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let before = app.buffer().snapshot().text();
    assert!(app.handle_event(&ime(ImeEvent::Started)).visual_changed);
    assert!(
...
#[test]
fn scroll_and_pointer_selection_use_rendered_visible_lines() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
...
#[test]
fn selection_spans_lines_and_unchanged_atlas_storage_is_reused() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    ...
    let second_atlas = second.glyph_atlas().ok_or(EditorRenderError::Domain)?;
...
-> Result<(), EditorRenderError> {
    let mut empty =
        EditorApp::new(UnresolvedEmptyTextSystem).map_err(|_| EditorRenderError::Domain)?;
    ...
    let mut failing = EditorApp::new(FailingTextSystem).map_err(|_| EditorRenderError::Domain)?;
    ...
    let text_error: EditorRenderError = TextError::EmptySelectionSet.into();
    let find_error: EditorRenderError = FindError::IncompleteResult.into();
    let layout_error: EditorRenderError = LayoutError::InvalidScroll.into();
    let scene_error: EditorRenderError = SceneError::MissingGlyphAtlas.into();
    assert!(
        text_error
            .to_string()
    ...
    assert_eq!(
        EditorRenderError::Domain.to_string(),
...
fn input_edges_are_bounded_and_failed_edits_are_atomic() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let next_epoch = InputEpoch::INITIAL
        .checked_next()
        .ok_or(EditorRenderError::Domain)?;
...
#[test]
fn pointer_drag_focus_and_outside_geometry_are_deterministic() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let _scene = app.try_scene(
        SceneRevision::new(1),
        viewport().map_err(|_| EditorRenderError::Domain)?,
    ...
    assert!(
        !app.handle_event(&SurfaceEvent::Pointer {
            ...
            position: Point::new(0.0, 0.0).ok_or(EditorRenderError::Domain)?,
            button: PointerButton::Primary,
            modifiers: Modifiers::default(),
        })
        .visual_changed
    ...
    assert!(
        !app.handle_event(&SurfaceEvent::Pointer {
            ...
            action: PointerAction::Down,
            position: Point::new(0.0, -1.0).ok_or(EditorRenderError::Domain)?,
    ...
    assert!(
        !app.handle_event(&SurfaceEvent::Pointer {
            ...
            action: PointerAction::Down,
            position: Point::new(CONTENT_INSET, huge_y).ok_or(EditorRenderError::Domain)?,
            button: PointerButton::Primary,
            modifiers: Modifiers::default(),
        })
        .visual_changed
    ...
    assert_eq!(
        app.offset_at_point(
            Point::new(CONTENT_INSET, CONTENT_INSET - 1.0).ok_or(EditorRenderError::Domain)?,
        ),
        Some(ByteOffset::new(0))
    );
    Ok(())
...
#[test]
fn coordinate_helpers_cover_clusters_bounds_and_binary_line_search() -> Result<(), EditorRenderError>
{
    ...
    let app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let snapshot = Buffer::new("a\nb\nc\nd\n").snapshot();
    assert_eq!(EditorApp::line_for_offset(&snapshot, 0)?, Some(0));
    assert_eq!(EditorApp::line_for_offset(&snapshot, 4)?, Some(2));
    assert_eq!(
        EditorApp::line_for_offset(&snapshot, snapshot.len_bytes())?,
        Some(snapshot.line_count() - 1)
    );
    assert_eq!(
        EditorApp::line_for_offset(&snapshot, snapshot.len_bytes() + 1)?,
        None
    );
...
#[test]
fn offscreen_caret_and_invalid_scroll_use_safe_scene_fallback() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    *app.buffer_mut() = Buffer::new(&"line\n".repeat(100));
    app.selection = Selection::caret(ByteOffset::new(0));
...
-> Result<(), EditorRenderError> {
    ...
    *app.buffer_mut() = Buffer::new("alpha beta alpha gamma alpha\n");
    ...
    let scene_viewport = viewport().map_err(|_| EditorRenderError::Domain)?;
    ...
    let expected_overlay = Rect::new(
        ...
        .ok_or(EditorRenderError::Domain)?,
        Size::new(FIND_BAR_WIDTH, FIND_BAR_HEIGHT).ok_or(EditorRenderError::Domain)?,
    );
    assert_eq!(
        before
            .clips()
...
#[test]
fn find_command_modifiers_do_not_edit_or_switch_fields() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let command = Modifiers::from_bits(Modifiers::COMMAND);
    let command_option = Modifiers::from_bits(Modifiers::COMMAND | Modifiers::OPTION);
    app.handle_event(&key(KEY_F, command));
...
#[test]
fn stale_find_completion_preserves_the_current_selection() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    *app.buffer_mut() = Buffer::new("alpha beta alpha");
    assert!(
        app.handle_event(&key(KEY_F, Modifiers::from_bits(Modifiers::COMMAND),))
...
#[test]
fn current_and_all_replacement_are_atomic_and_undoable() -> Result<(), EditorRenderError> {
    let original = "alpha beta alpha";
    let command = Modifiers::from_bits(Modifiers::COMMAND);
    let command_option = Modifiers::from_bits(Modifiers::COMMAND | Modifiers::OPTION);

    let mut current = test_app().map_err(|_| EditorRenderError::Domain)?;
    *current.buffer_mut() = Buffer::new(original);
    ...
    let mut all = test_app().map_err(|_| EditorRenderError::Domain)?;
...
fn find_focus_rejections_and_empty_actions_remain_bounded() -> Result<(), EditorRenderError> {
    let mut app = test_app().map_err(|_| EditorRenderError::Domain)?;
    let command = Modifiers::from_bits(Modifiers::COMMAND);
    let command_option = Modifiers::from_bits(Modifiers::COMMAND | Modifiers::OPTION);
    assert!(app.handle_event(&key(KEY_F, command)).visual_changed);
    ...
    *app.buffer_mut() = Buffer::new(&"line\n".repeat(100));
    app.last_viewport = viewport().map_err(|_| EditorRenderError::Domain)?;
    app.scroll_y = 50.0;
    ...
    let mut empty = test_app().map_err(|_| EditorRenderError::Domain)?;
    *empty.buffer_mut() = Buffer::new("alpha");
    ...
    let mut budget = test_app().map_err(|_| EditorRenderError::Domain)?;
    *budget.buffer_mut() = Buffer::new(&"x".repeat(5_000));
    ...
    let mut exhausted = test_app().map_err(|_| EditorRenderError::Domain)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let root = TestWorkspace::new()?;
    root.write("alpha.rs", "alpha")?;
    let app = EditorApp::open_workspace(TestTextSystem, root.path())?;
    let viewport = viewport()?;
    let clear = LinearRgba::new(0.02, 0.02, 0.02, 1.0).ok_or(SurfaceError::invariant(
        alpine_platform_macos::SurfaceOperation::Application,
    ))?;
...
fn run_rejects_an_unsupported_host() {
    ...
    assert!(matches!(
        ...
        Err(EditorError::Runtime(RuntimeError::Surface(
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        ...
        Err(EditorError::Workspace(WorkspaceError::Io {
...
-> Result<(), Box<dyn Error>> {
    let document = EditorDocument::scratch("semantic revision");
    let mut app = EditorApp::from_document(TestTextSystem, document, None)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut exhausted_open = EditorApp::open_workspace(TestTextSystem, root.path())?;
    ...
    let mut missing = EditorApp::open_workspace(TestTextSystem, root.path())?;
    ...
    let position = Point::new(20.0, 20.0).ok_or(EditorRenderError::Domain)?;
    assert_eq!(
    ...
    let mut ime_app = EditorApp::open_workspace(TestTextSystem, root.path())?;
    ...
    let mut delete_error = EditorApp::open_workspace(TestTextSystem, root.path())?;
    assert!(
        delete_error
            .handle_event(&key(KEY_P, command))
            .visual_changed
    ...
    let mut stale = EditorApp::open_workspace(TestTextSystem, root.path())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = TestWorkspace::new()?;
    root.write("alpha.rs", "alpha")?;
    let mut query_app = EditorApp::open_workspace(
    ...
    assert!(matches!(
        ...
        Err(EditorRenderError::Layout(LayoutError::NativeFailure(_)))
    ));

    let nested = TestWorkspace::new()?;
    nested.create_dir("src")?;
    nested.write("src/main.rs", "main")?;
    let mut success_app = EditorApp::open_workspace(TestTextSystem, nested.path())?;
    assert!(
        success_app
            .handle_event(&key(KEY_P, command))
            .visual_changed
    ...
    let mut row_app = EditorApp::open_workspace(
        SelectiveFailingRasterTextSystem {
            glyph_id: u32::from('/'),
        },
        nested.path(),
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = TestWorkspace::new()?;
    root.write("alpha.rs", "alpha")?;
    let mut rejected = EditorApp::open_workspace(TestTextSystem, root.path())?;
    let command = Modifiers::from_bits(Modifiers::COMMAND);
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut admitted = EditorApp::open_workspace_lazy(TestTextSystem, admitted_root.path())?;
    assert!(
        admitted
    ...
    let mut rejected = EditorApp::open_workspace_lazy(TestTextSystem, rejected_root.path())?;
    assert!(
        rejected
            .handle_event(&key(KEY_E, command_shift))
            .visual_changed
...
-> Result<(), Box<dyn std::error::Error>> {
    let render_error = EditorRenderError::from(FileTreeError::NoWorkspace);
    assert!(
        render_error
            .to_string()
...
-> Result<(), Box<dyn std::error::Error>> {
    let root = TestWorkspace::new()?;
    root.write("a.rs", "a")?;
    let mut pointer_app = EditorApp::open_workspace_lazy(TestTextSystem, root.path())?;
    ...
    let mut exhausted = EditorApp::open_workspace_lazy(TestTextSystem, root.path())?;
    ...
    let mut stale = EditorApp::open_workspace_lazy(TestTextSystem, stale_root.path())?;
    ...
    let mut failed = EditorApp::open_workspace_lazy(TestTextSystem, failed_root.path())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut app = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("a🦀é"), None)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("a🦀é"), None)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut oversized = EditorApp::from_document(
        TestTextSystem,
        EditorDocument::scratch(&"é".repeat(32_769)),
        None,
    )?;
...
fn assert_completion_shutdown_and_idle_are_bounded(
    ...
    let mut idle_app = EditorApp::open_file(TestTextSystem, path)?;
...
fn assert_completion_trigger_guards(app: &mut EditorApp) {
    app.composition = Some(Composition {
        replacement: app.selection.range(),
        text: Box::default(),
        selected_start_utf16: 0,
    ...
    assert!(
        app.dispatch_command(EditorCommand::TriggerCompletion)
            .visual_changed
    );
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut fallback_app = EditorApp::open_file(TestTextSystem, &deep_path)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let plain = EditorDocument::scratch("plain text");
    let plain = EditorApp::from_document(TestTextSystem, plain, Some(Path::new("notes.txt")))?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut changed =
        EditorApp::from_document(TestTextSystem, EditorDocument::scratch("a\nb"), None)?;
    ...
    let mut find = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("x"), None)?;
    assert!(find.find.open(false));
    ...
    let mut project = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("x"), None)?;
    ...
    let mut tree = EditorApp::from_document(TestTextSystem, EditorDocument::scratch("x"), None)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = EditorApp::from_document(TestTextSystem, EditorDocument::scratch(&text), None)?;
    let snapshot = app.accessibility_snapshot()?;
    assert_eq!(
        snapshot
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = EditorApp::from_workspace(TestTextSystem, workspace)?;
...
impl AppDelegate for WorkerContinuationObserver {
    ...
    fn worker_result(
        ...
        let find_result = matches!(&result, EditorWorkerOutput::Find(_));
        if find_result {
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let text_app = EditorApp::open_file(TestTextSystem, &text_path)?;
    ...
    let mut projected_app = EditorApp::open_file(TestTextSystem, &rust_path)?;
    ...
    let mut latched_app = EditorApp::open_file(TestTextSystem, &rust_path)?;
