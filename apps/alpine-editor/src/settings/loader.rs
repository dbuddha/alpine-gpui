...
use super::{
    AppSettings, EditorSettingsPatch, EditorTheme, FONT_NAME, KeyAction, KeyBinding, Keymap,
    MAX_KEY_BINDINGS, SettingsAdmission, SettingsEffect, SettingsFailure, SettingsLayer,
    SettingsSource, SettingsState, SettingsUpdate, SyntaxTheme, color,
};
...
fn load_settings(
    ...
    let compiled = AppSettings::compiled().map_err(|_| compiled_settings_failure())?;
...
fn decode_theme(value: &Value, base_theme: &EditorTheme) -> Result<EditorTheme, SettingsLoadError> {
    let object = value
        .as_object()
...
fn decode_command(value: &str) -> Option<EditorCommand> {
    match value {
        "save_file" => Some(EditorCommand::SaveFile),
        "close_tab" => Some(EditorCommand::CloseTab),
        "navigate_back" => Some(EditorCommand::NavigateBack),
        "navigate_forward" => Some(EditorCommand::NavigateForward),
        "activate_previous_tab" => Some(EditorCommand::ActivatePreviousTab),
        "activate_next_tab" => Some(EditorCommand::ActivateNextTab),
        "open_quick_open" => Some(EditorCommand::OpenQuickOpen),
        ...
        "split_down" => Some(EditorCommand::SplitDown),
        "focus_next_pane" => Some(EditorCommand::FocusNextPane),
        "close_pane" => Some(EditorCommand::ClosePane),
        _ => None,
    }
}
...
mod tests {
    ...
    fn global_then_project_and_migration_are_atomic() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(
            ...
            Some(KeyAction::Command(EditorCommand::ReloadSettings))
        );
    ...
    -> Result<(), Box<dyn Error>> {
        ...
        let (layer, report) = decode_layer(&bytes, &EditorTheme::compiled()?)?;
    ...
    fn independent_loader_bounds_fail_closed() -> Result<(), Box<dyn Error>> {
        ...
        .to_string();
        let theme = EditorTheme::compiled()?;
        assert_eq!(
        ...
        let (legacy_layer, _) = decode_layer(&legacy_bytes, &EditorTheme::compiled()?)?;
    ...
    fn file_and_json_boundaries_fail_closed() -> Result<(), Box<dyn Error>> {
        ...
        assert!(matches!(
            decode_layer(deep, &EditorTheme::compiled()?),
