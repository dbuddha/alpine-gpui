...
use super::{
    EditorApp, EditorCommand, EditorWorkerOutput, KEY_DELETE_BACKWARD, KEY_ESCAPE, KEY_RETURN,
    LocalStatus, WorkspaceEditApplicationError, WorkspaceEditKind, WorkspaceEditPublicationError,
...
-> Result<(), Box<dyn std::error::Error>> {
    let (root, path, _, _) = fixture();
    let mut admitted = EditorApp::open_file(TestTextSystem, &path)?;
    ...
    drop(admitted);
    let mut scratch = EditorApp::new(TestTextSystem)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = EditorApp::open_file(TestTextSystem, &first_path)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        app.tabs
            .inactive_document_for_path(&first_path)
            .is_some_and(super::EditorDocument::is_dirty)
    );
    drop(app);
    fs::remove_dir_all(root)?;
    Ok(())
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        app.dispatch_command(EditorCommand::PreviewRustRename)
            .visual_changed
    );
...
-> Result<(), Box<dyn std::error::Error>> {
    let (root, path, _, _) = fixture();
    let mut formatting_app = EditorApp::open_file(TestTextSystem, &path)?;
    ...
    let formatting_command = formatting_app.dispatch_command(EditorCommand::PreviewRustFormatting);
...
impl AppDelegate for WorkspaceHandoffObserver {
    ...
    fn worker_result(
        ...
        let expected = matches!(
            ...
            (true, EditorWorkerOutput::WorkspaceEdit(_))
                | (false, EditorWorkerOutput::WorkspaceEditPublication(_))
        );
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut preparation_app = EditorApp::open_file(TestTextSystem, &path)?;
    ...
    let mut publication_app = EditorApp::open_file(TestTextSystem, &path)?;
    ...
    let mut rejected_preparation_app = EditorApp::open_file(TestTextSystem, &path)?;
    assert!(
    ...
    let mut rejected_publication_app = EditorApp::open_file(TestTextSystem, &path)?;
    assert!(
    ...
    let mut missing_persistence_app = EditorApp::open_file(TestTextSystem, &path)?;
    assert!(
