...
use crate::{commands::EditorCommand, syntax::SyntaxClass};
...
static DEFAULT_BINDINGS: [KeyBinding; 22] = [
    ...
    binding(
        KEY_F,
        COMMAND_SHIFT,
        KeyAction::Command(EditorCommand::OpenProjectSearch),
        "Cmd+Shift+F",
    ),
    binding(
        KEY_SPACE,
        Modifiers::CONTROL,
        KeyAction::Command(EditorCommand::TriggerCompletion),
        "Ctrl+Space",
    ),
    binding(
        KEY_E,
        COMMAND_SHIFT,
        KeyAction::Command(EditorCommand::ToggleFileTree),
        "Cmd+Shift+E",
    ),
    binding(
        KEY_F,
        COMMAND_OPTION,
        KeyAction::Command(EditorCommand::OpenReplace),
        "Cmd+Opt+F",
    ),
    binding(
        KEY_P,
        Modifiers::COMMAND,
        KeyAction::Command(EditorCommand::OpenQuickOpen),
        "Cmd+P",
    ),
    binding(
        KEY_F,
        Modifiers::COMMAND,
        KeyAction::Command(EditorCommand::OpenFind),
        "Cmd+F",
    ),
    ...
    binding(
        KEY_S,
        Modifiers::COMMAND,
        KeyAction::Command(EditorCommand::SaveFile),
        "Cmd+S",
    ),
    ...
    binding(
        KEY_LEFT_BRACKET,
        Modifiers::COMMAND,
        KeyAction::Command(EditorCommand::NavigateBack),
        "Cmd+[",
    ),
    binding(
        KEY_RIGHT_BRACKET,
        Modifiers::COMMAND,
        KeyAction::Command(EditorCommand::NavigateForward),
        "Cmd+]",
    ),
];
...
pub(crate) struct SettingsLayer {
    ...
    pub(crate) theme: Option<EditorTheme>,
...
impl SettingsEffect {
    fn between(current: &AppSettings, candidate: &AppSettings) -> Self {
        Self {
...
impl SettingsState {
    pub(crate) fn compiled() -> Result<Self, SettingsError> {
        let active = AppSettings::compiled()?;
    ...
    pub(crate) const fn active(&self) -> &AppSettings {
        &self.active
    }
...
fn resolve_layers(
    ...
    resolve_layers_from(AppSettings::compiled_with_retained(), global, project)
}

fn resolve_layers_from(
    compiled: Result<(AppSettings, usize), SettingsError>,
    global: Option<&SettingsLayer>,
    project: Option<&SettingsLayer>,
) -> Result<ResolvedSettings, SettingsFailure> {
...
mod tests {
    ...
    fn compiled_settings_are_valid_static_and_bounded() -> Result<(), SettingsError> {
        ...
        assert_eq!(
            settings.keymap.shortcut_for(EditorCommand::OpenFind),
            Some("Cmd+F")
        );
        assert_eq!(
            settings.keymap.shortcut_for(EditorCommand::SplitRight),
            None
        );
    ...
    -> Result<(), SettingsError> {
        ...
        assert_eq!(
            ...
            Some(KeyAction::Command(EditorCommand::OpenProjectSearch))
        );
        assert_eq!(
            keymap.resolve(
                KEY_S,
            ...
            Some(KeyAction::Command(EditorCommand::SaveFile))
        );
    ...
    -> Result<(), SettingsError> {
        ...
        let mut project_theme = EditorTheme::compiled()?;
    ...
    fn layering_limits_and_retained_accounting_are_exact() -> Result<(), SettingsError> {
        ...
        let mut exact_budget = AppSettings::compiled()?;
        ...
        let mut over_budget = AppSettings::compiled()?;
