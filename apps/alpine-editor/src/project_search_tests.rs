...
fn open_and_search(app: &mut EditorApp, root: &std::path::Path) -> Result<(), Box<dyn Error>> {
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut missing = EditorApp::new(SearchTextSystem)?;
    let effect = missing.dispatch_command(EditorCommand::OpenProjectSearch);
    assert!(effect.visual_changed);
    assert!(
        missing
            .local_status
...
-> Result<(), Box<dyn std::error::Error>> {
    let render: EditorRenderError = ProjectSearchError::InvalidLimits.into();
    assert!(
        render
            .to_string()
    ...
    let project = TempProject::new()?;
    let mut invalid = EditorApp::open_workspace_lazy(SearchTextSystem, &project.root)?;
    ...
    let mut exhausted = EditorApp::open_workspace_lazy(SearchTextSystem, &project.root)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for rejected_glyph in [u32::from('P'), u32::from('~')] {
        let mut app = EditorApp::open_workspace_lazy(
        ...
        assert!(matches!(
            ...
            Err(EditorRenderError::Layout(_))
        ));
    }

    let mut clip = EditorApp::open_workspace_lazy(FaultSearchTextSystem::default(), &project.root)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let project = TempProject::new()?;
    let app = EditorApp::open_workspace_lazy(SearchTextSystem, &project.root)?;
    ...
    let mut rejected = EditorApp::open_workspace_lazy(SearchTextSystem, &project.root)?;
