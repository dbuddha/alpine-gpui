...
const CAPTURED_AT_ENV: &str = "ALPINE_EDITOR_DOGFOOD_CAPTURED_AT_UTC";
#[cfg(all(test, not(miri)))]
const ENVIRONMENT_TEST_CHILD_ENV: &str = "ALPINE_EDITOR_DOGFOOD_TEST_CHILD";
...
#[derive(Clone)]
pub(super) struct CaptureSink(Rc<RefCell<Option<EditorOwnedSnapshot>>>);

impl CaptureSink {
    pub(super) fn capture(&self, app: &EditorApp) {
        if let Ok(mut slot) = self.0.try_borrow_mut()
            && slot.is_none()
        {
            *slot = Some(EditorOwnedSnapshot::capture(app));
        }
    }
}
...
impl EditorOwnedSnapshot {
    fn capture(app: &EditorApp) -> Self {
...
fn render_payload(
    ...
    json!({
        "schema": "alpine-editor-internal-diagnostic/v1",
...
fn language_evidence(owned: EditorOwnedSnapshot) -> Value {
    json!({
...
fn accessibility_evidence(owned: EditorOwnedSnapshot) -> Value {
    json!({
...
fn omission_inventory(owned: EditorOwnedSnapshot) -> Value {
...
pub(super) fn capture_surface_error(error: &CaptureError) -> alpine_platform_macos::SurfaceError {
    eprintln!("Alpine Editor dogfood diagnostic rejected: {error}");
...
mod tests {
    ...
    fn unavailable_axes_are_null_and_explicitly_omitted() {
        let owned = EditorOwnedSnapshot::default();
        let payload = json!({
            "language": language_evidence(owned),
            "accessibility": accessibility_evidence(owned),
    ...
    #[test]
    fn real_editor_capture_finishes_one_bounded_payload() -> Result<(), String> {
        ...
        assert_eq!(
            payload.get("schema"),
            Some(&json!("alpine-editor-internal-diagnostic/v1"))
        );
