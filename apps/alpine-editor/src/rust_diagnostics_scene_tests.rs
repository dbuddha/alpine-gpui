...
fn assert_invalid_diagnostic_origin(app: &mut EditorApp, viewport: Size) {
    app.diagnostic_origin_x_override = Some(f32::NAN);
    assert!(matches!(
        app.try_scene(SceneRevision::new(4), viewport),
        Err(EditorRenderError::Domain)
    ));
...
-> Result<(), EditorRenderError> {
    let normal = Rect::new(
        Point::new(20.0, 10.0).ok_or(EditorRenderError::Domain)?,
        Size::new(400.0, 360.0).ok_or(EditorRenderError::Domain)?,
    );
    assert_eq!(
        EditorApp::language_overlay_bounds(normal, 2)?,
        Rect::new(
            Point::new(44.0, 58.0).ok_or(EditorRenderError::Domain)?,
            Size::new(352.0, 44.0).ok_or(EditorRenderError::Domain)?,
        )
    );

    let bottom_limited = Rect::new(
        Point::new(20.0, 10.0).ok_or(EditorRenderError::Domain)?,
        Size::new(400.0, 70.0).ok_or(EditorRenderError::Domain)?,
    );
    assert_eq!(
        EditorApp::language_overlay_bounds(bottom_limited, 2)?.origin(),
        Point::new(44.0, 36.0).ok_or(EditorRenderError::Domain)?
    );

    let constrained = Rect::new(
        Point::new(5.0, 0.0).ok_or(EditorRenderError::Domain)?,
        Size::new(10.0, 20.0).ok_or(EditorRenderError::Domain)?,
    );
    assert_eq!(
        EditorApp::language_overlay_bounds(constrained, 1)?,
        Rect::new(
            Point::new(14.0, 0.0).ok_or(EditorRenderError::Domain)?,
            Size::new(1.0, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?,
        )
    );
    Ok(())
}
...
-> Result<(), Box<dyn Error>> {
    ...
    let mut app = EditorApp::open_file(tests::TestTextSystem, &path)?;
    ...
    let pane = app
        ...
        .ok_or("active pane")?
        .bounds;
    let hover_bounds = EditorApp::language_overlay_bounds(pane, 2)?;
    ...
    let location_bounds = EditorApp::language_overlay_bounds(pane, 2)?;
...
fn navigation_overlay_raster_failures_preserve_scene_atomicity() -> Result<(), Box<dyn Error>> {
    ...
    let mut hover_app = EditorApp::open_file(
    ...
    let mut location_app = EditorApp::open_file(
        FailingRasterTextSystem {
            glyph_id: u32::from('§'),
        },
        &path,
...
fn symbol_overlay_raster_failures_preserve_query_and_row_atomicity() -> Result<(), Box<dyn Error>> {
    ...
    let install = |app: &mut EditorApp, values: &RawValue| {
        let input = app.active_rust_document().ok_or("Rust document")?;
        app.rust_diagnostics.install_for_test(
            input,
            &rust_diagnostics::tests::diagnostics(&path, 1),
    ...
    let mut query_app = EditorApp::open_file(
        FailingRasterTextSystem {
            glyph_id: u32::from('Ω'),
        },
        &path,
    ...
    let mut row_app = EditorApp::open_file(
        FailingRasterTextSystem {
            glyph_id: u32::from('§'),
        },
        &path,
