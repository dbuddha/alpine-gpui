...
use crate::{EditorApp, EventEffect};
...
impl Owner {
    pub(super) fn active(app: &EditorApp) -> Option<Self> {
    ...
    pub(super) fn node(self, app: &EditorApp) -> AccessibilityNodeId {
    ...
    pub(super) fn apply(self, app: &mut EditorApp, prepared: Prepared) -> EventEffect {
    ...
    pub(super) fn commit(self, app: &mut EditorApp, text: &str, caret: usize) -> EventEffect {
        let prepared = (|| {
    ...
    pub(super) fn reject(self, app: &mut EditorApp, error: EditError) -> EventEffect {
        if let Some((_, edit)) = self.parts(app) {
            edit.cancel_composition();
        }
        if self == Self::Rename {
...
impl PendingClipboard {
    ...
    fn current(&self, app: &EditorApp) -> bool {
        self.valid
...
impl EditorApp {
    pub(super) fn begin_field_clipboard(
        ...
        let Some((value, edit)) = owner.read(self) else {
            return crate::EditorTransition::default();
        };
        ...
        match write {
            ...
            Err(error) => crate::EditorTransition::effect(self.record_clipboard_error(error)),
        }
    }
...
mod tests {
    ...
    fn open(app: &mut EditorApp, owner: Owner) -> Result<(), Box<dyn std::error::Error>> {
        match owner {
            Owner::Find => {
                app.find.open(false);
            }
