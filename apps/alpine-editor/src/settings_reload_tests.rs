...
-> Result<(), Box<dyn Error>> {
    ...
    let mut app = EditorApp::open_workspace(tests::TestTextSystem, root.path())?;
...
fn command_reload_announces_but_startup_keymap_only_does_not_redraw() -> Result<(), Box<dyn Error>>
{
    ...
    let mut app = EditorApp::new(tests::TestTextSystem)?;
    ...
    assert_eq!(
        app.dispatch_command(EditorCommand::ReloadSettings),
        EventEffect::visual()
    );
    let manual = app
        .settings_reload
