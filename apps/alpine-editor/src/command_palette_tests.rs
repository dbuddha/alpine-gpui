...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = EditorApp::open_file(PaletteTextSystem::default(), &file.path)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut app = EditorApp::new(PaletteTextSystem {
    ...
    assert!(matches!(
        ...
        Err(EditorRenderError::Layout(LayoutError::InvalidShaperOutput))
    ));
    Ok(())
}
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = EditorApp::open_workspace_lazy(PaletteTextSystem::default(), &file.root)?;
    ...
    let _ = app.dispatch_command(EditorCommand::SaveFile);
    let _ = app.dispatch_command(EditorCommand::NavigateBack);
    assert!(app.command_context().can_navigate_forward);
    let _ = app.dispatch_command(EditorCommand::NavigateForward);
    let _ = app.dispatch_command(EditorCommand::OpenFind);
    assert!(app.find.is_open());
    app.find.close();
    let _ = app.dispatch_command(EditorCommand::OpenReplace);
    assert!(app.find.is_open());
    app.find.close();
    let _ = app.dispatch_command(EditorCommand::OpenQuickOpen);
    assert!(app.quick_open.is_open());
    app.quick_open.close();
    let _ = app.dispatch_command(EditorCommand::ToggleFileTree);
    assert!(app.file_tree.is_visible());
    assert!(app.file_tree.is_focused());
    let _ = app.dispatch_command(EditorCommand::ToggleFileTree);
    assert!(!app.file_tree.is_visible());
    let before = app.tabs.len();
    let _ = app.dispatch_command(EditorCommand::CloseTab);
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        app.dispatch_command(EditorCommand::OpenQuickOpen)
            .visual_changed
    );
    assert!(
        app.dispatch_command(EditorCommand::ToggleFileTree)
            .visual_changed
    );
...
fn command_dispatch_preserves_bounded_workspace_errors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut app = EditorApp::open_workspace_lazy(PaletteTextSystem::default(), &workspace.root)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    app.open_workspace_path(&second, None)?;
    assert!(app.open_command_palette().visual_changed);
    assert_eq!(app.command_palette.visible_commands()?.len(), 15);
    ...
    let overlay = Rect::new(
        ...
        Size::new(252.0, 394.0).ok_or("overlay size")?,
    );
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut scratch = EditorApp::new(PaletteTextSystem::default())?;
    assert!(
        scratch
    ...
    assert!(app.command_context().can_close_tab);
    let before = app.buffer().snapshot().text();
    assert!(app.dispatch_command(EditorCommand::CloseTab).visual_changed);
...
-> Result<(), Box<dyn std::error::Error>> {
    let mut missing = EditorApp::new(PaletteTextSystem::default())?;
    let _ = missing.dispatch_command(EditorCommand::OpenQuickOpen);
    assert!(
        missing
            .local_status
            .as_ref()
    ...
    let _ = missing.dispatch_command(EditorCommand::ToggleFileTree);
    assert!(
        missing
            .local_status
            .as_ref()
    ...
    for command in [EditorCommand::OpenFind, EditorCommand::OpenReplace] {
        let mut app = EditorApp::new(PaletteTextSystem::default())?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    app.dispatch_command(EditorCommand::OpenFind);
    ...
    app.find_needs_search = false;
    app.dispatch_command(EditorCommand::OpenReplace);
...
-> Result<(), Box<dyn std::error::Error>> {
    let converted = EditorRenderError::from(CommandPaletteError::InvalidComposition);
    assert!(converted.to_string().contains("command-palette rendering"));

    let mut query_failure = EditorApp::new(PaletteTextSystem {
        rejected_glyph: Some(u32::from('>')),
    })?;
    ...
    assert!(matches!(query_result, Err(EditorRenderError::Layout(_))));

    let mut row_failure = EditorApp::new(PaletteTextSystem {
    ...
    assert!(matches!(row_result, Err(EditorRenderError::Layout(_))));

    let mut scene_failure = EditorApp::new(PaletteTextSystem::default())?;
    ...
    assert!(matches!(
        scene_result,
        Err(EditorRenderError::Scene(SceneError::InvalidClip { .. }))
    ));
    Ok(())
}
