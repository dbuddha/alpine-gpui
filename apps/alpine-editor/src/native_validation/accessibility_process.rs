...
use super::{
    COMMAND_SHIFT_MODIFIERS, DEFAULT_SCALE, EditorApp, EditorError, FONT_FAMILY, KEY_E, KEY_P,
    PresentationEvidenceMode, WINDOW_HEIGHT, WINDOW_WIDTH, Workspace, event_handler,
    keyboard_event, parse_presentation_evidence_mode,
};
...
impl OmittedStep {
    ...
    fn from_environment() -> Result<Option<Self>, Box<dyn std::error::Error>> {
        let Ok(value) = std::env::var("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_OMIT") else {
            return Ok(None);
        };
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct NativeEditorAccessibilityEvidence {
    tree_actions: usize,
    tab_actions: usize,
...
impl NativeEditorAccessibilityEvidence {
...
pub fn qualify_editor_accessibility_process()
-> Result<NativeEditorAccessibilityEvidence, Box<dyn std::error::Error>> {
    ...
    let root = std::env::temp_dir().join(format!(
        "alpine-editor-native-accessibility-{}-{nonce}",
        std::process::id()
    ));
...
fn record_qualification_child_phase(server: &OsStr) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let expected_process = std::env::var_os("ALPINE_EDITOR_NATIVE_PROCESS_EXE")
        .ok_or("ALPINE_EDITOR_NATIVE_PROCESS_EXE is required for native Studio qualification")?;
    ...
    let trace = std::env::var_os("ALPINE_EDITOR_NATIVE_LSP_TRACE")
        .ok_or("ALPINE_EDITOR_NATIVE_LSP_TRACE is required for native Studio qualification")?;
...
fn qualification_wrapper_valid(wrapper: &str) -> bool {
    wrapper.contains("wrapper-invoked") && wrapper.contains("ALPINE_EDITOR_NATIVE_LSP_SERVER")
}
...
fn qualify_workspace(
    ...
    let descriptor = SurfaceDescriptor::new(
        "Alpine Editor native accessibility process",
    ...
    Ok(NativeEditorAccessibilityEvidence {
        tree_actions,
        tab_actions,
        command_actions,
        diagnostic_actions,
...
fn language_server_phase_trace() -> Box<str> {
    let Some(path) = std::env::var_os("ALPINE_EDITOR_NATIVE_LSP_TRACE") else {
...
mod process_contract_tests {
    ...
    fn process_evidence_accessors_preserve_nondefault_values() {
        let evidence = NativeEditorAccessibilityEvidence {
            tree_actions: 2,
            tab_actions: 3,
            command_actions: 4,
    ...
    fn process_language_ownership_requires_every_independent_boundary() {
        assert!(qualification_wrapper_valid(
            "wrapper-invoked ALPINE_EDITOR_NATIVE_LSP_SERVER"
        ));
        assert!(!qualification_wrapper_valid("wrapper-invoked"));
        assert!(!qualification_wrapper_valid(
            "ALPINE_EDITOR_NATIVE_LSP_SERVER"
        ));
