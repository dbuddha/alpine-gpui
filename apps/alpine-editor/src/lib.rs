...
//! Local-only Alpine Editor editor boundary.
...
/// A structured Alpine Editor launch failure.
#[derive(Debug)]
...
impl fmt::Display for EditorError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Usage => formatter.write_str("usage: alpine-editor [path]"),
...
impl From<FileError> for EditorError {
    fn from(error: FileError) -> Self {
        Self::File(error)
    }
}

impl From<WorkspaceError> for EditorError {
...
impl From<SurfaceError> for EditorError {
...
#[cfg_attr(test, mutants::skip)] // Entering AppKit is qualified by native process E2E.
pub fn run_file(path: impl AsRef<Path>) -> Result<(), EditorError> {
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
...
pub fn run_path(path: impl AsRef<Path>) -> Result<(), EditorError> {
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
        run_native(native_explicit_path_app(
            path.as_ref(),
...
#[cfg(all(target_os = "macos", target_arch = "aarch64"))]
#[cfg_attr(test, mutants::skip)] // Entering AppKit is qualified by native process E2E.
fn run_native(app: EditorApp) -> Result<(), RuntimeError> {
    #[cfg(alpine_native_validation)]
    let mut app = app;
    ...
    #[cfg(alpine_native_validation)]
    let production_process_scenario = std::env::var_os("ALPINE_EDITOR_NATIVE_PROCESS_SCENARIO");
    #[cfg(alpine_native_validation)]
    ...
    if production_process_validation {
        ...
        let expected = std::env::var_os("ALPINE_EDITOR_NATIVE_EXPECTED_PATH");
...
fn native_app() -> Result<EditorApp, SurfaceError> {
...
fn native_restored_app() -> Result<EditorApp, SurfaceError> {
    ...
    let recovery_warning = match recovery::load(&recovery::path_for_session(&path)) {
        Ok(state) => {
            ...
            match EditorApp::from_recovery(text_system, state) {
                Ok(mut app) => {
    ...
    let mut app = match session::load(&path) {
        Ok(state) => {
            ...
            match EditorApp::from_session(text_system, state) {
                Ok(app) => app,
...
fn session_fallback(detail: &str) -> Result<EditorApp, SurfaceError> {
    let mut app = native_app()?;
...
impl ExplicitPathTarget {
    fn open(path: &Path, kind: ExplicitPathKind) -> Result<Self, EditorError> {
    ...
    ) -> Result<EditorApp, EditorError> {
        match self {
            Self::File { path, document } => {
                EditorApp::from_document(text_system, document, Some(&path))
                    .map_err(EditorError::from)
            }
            Self::Workspace { workspace, .. } => {
                let mut app = EditorApp::from_workspace(text_system, workspace)?;
    ...
    fn compose(self, app: &mut EditorApp) -> Result<(), RecoveryLaunchError> {
...
fn recovery_editor_error(
    requested: &Path,
    journal: &Path,
    source: impl Into<RecoveryLaunchError>,
) -> EditorError {
    EditorError::Recovery {
...
fn map_recovery_restore_result<T>(
    ...
    result.map_err(|error| {
        recovery_editor_error(requested, journal, classify_recovery_restore_error(&error))
    })
}
...
fn compose_explicit_path(
    ...
    let recovery = match recovery::load(&journal) {
        ...
        Err(error) => return Err(recovery_editor_error(&requested, &journal, error)),
    };
    let mut app = if let Some(state) = recovery {
        let restored = EditorApp::from_recovery(text_system, state);
        let mut app = map_recovery_restore_result(restored, &requested, &journal)?;
        target
            .compose(&mut app)
            .map_err(|source| recovery_editor_error(&requested, &journal, source))?;
        app
    } else {
        target.into_app(text_system)?
    };
    app.configure_recovery_persistence(session_path)
        .map_err(|error| recovery_editor_error(&requested, &journal, error))?;
    Ok(app)
}

#[cfg(all(target_os = "macos", target_arch = "aarch64"))]
fn native_explicit_path_app(path: &Path, kind: ExplicitPathKind) -> Result<EditorApp, EditorError> {
...
fn recover_explicit_workspace_edit(
    ...
    let journal = rust_workspace_publish::journal_path_for_session(session_path).map_err(|_| {
        recovery_editor_error(launch_path, session_path, RecoveryLaunchError::Invalid)
    })?;
    rust_workspace_publish::recover_pending(&journal)
        .map_err(|_| recovery_editor_error(launch_path, &journal, RecoveryLaunchError::Invalid))?;
    Ok(())
}

trait EditorTextSystem: TextShaper + GlyphRasterizer {}

impl<T: TextShaper + GlyphRasterizer> EditorTextSystem for T {}
...
impl From<CommandPaletteError> for EditorRenderError {
    fn from(error: CommandPaletteError) -> Self {
        Self::CommandPalette(error)
    }
}

impl From<TextError> for EditorRenderError {
    fn from(error: TextError) -> Self {
        Self::Text(error)
    }
}

impl From<FileTreeError> for EditorRenderError {
    fn from(error: FileTreeError) -> Self {
        Self::FileTree(error)
    }
}

impl From<FindError> for EditorRenderError {
    fn from(error: FindError) -> Self {
        Self::Find(error)
    }
}

impl From<QuickOpenError> for EditorRenderError {
    fn from(error: QuickOpenError) -> Self {
        Self::QuickOpen(error)
    }
}

impl From<ProjectSearchError> for EditorRenderError {
    fn from(error: ProjectSearchError) -> Self {
        Self::ProjectSearch(error)
    }
}

impl From<SyntaxError> for EditorRenderError {
    fn from(error: SyntaxError) -> Self {
        Self::Syntax(error)
    }
}

impl From<LayoutError> for EditorRenderError {
    fn from(error: LayoutError) -> Self {
        Self::Layout(error)
    }
}

impl From<SceneError> for EditorRenderError {
    fn from(error: SceneError) -> Self {
        Self::Scene(error)
    }
}

impl fmt::Display for EditorRenderError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::CommandPalette(error) => {
...
impl Error for EditorRenderError {}
...
struct EditorApp {
    ...
    document: EditorDocument,
    tabs: DocumentTabs<EditorDocument>,
...
impl EditorApp {
    fn new(text_system: impl EditorTextSystem + 'static) -> Result<Self, SurfaceError> {
        Self::from_document(text_system, EditorDocument::scratch(INITIAL_TEXT), None)
    }
    ...
    fn open_file(
        ...
        let document = EditorDocument::open(path)?;
        Self::from_document(text_system, document, Some(path)).map_err(EditorError::from)
    }
    ...
    fn open_workspace(
        ...
        let mut app = Self::from_workspace(text_system, workspace).map_err(EditorError::from)?;
        app.file_tree
            .activate(1)
    ...
    fn open_workspace_lazy(
        ...
        Self::from_workspace(text_system, workspace).map_err(EditorError::from)
    }
    ...
    fn initial_tabs_and_panes(
        path: Option<&Path>,
    ) -> Result<(DocumentTabs<EditorDocument>, PaneGrid), SurfaceError> {
    ...
    fn from_session_with_recovery(
        ...
        let active_document = if let Some(recovered) = recoveries[active_index].as_ref() {
            let document = EditorDocument::recover(active.path.as_deref(), recovered)
            ...
            active
                .path
                .as_deref()
                .map(|path| EditorDocument::open_for_restore(path, availability))
                .transpose()
                .map_err(|_| SessionRestoreError::File)?
                .unwrap_or_else(|| EditorDocument::scratch(INITIAL_TEXT))
        };
    ...
    fn ensure_document_tab_loaded(&mut self, index: usize) -> Result<(), WorkspaceSelectionError> {
        ...
        let document =
            if let Some(recovered) = self.pending_recovery.get(index).and_then(Option::as_ref) {
                let document = EditorDocument::recover(self.tabs.path_at(index), recovered)
                    .map_err(|_| WorkspaceSelectionError::RevisionExhausted)?;
                self.pending_recovery[index] = None;
                document
            } else {
                self.tabs.path_at(index).map_or_else(
                    || Ok(EditorDocument::scratch(INITIAL_TEXT)),
                    |path| {
                        EditorDocument::open_for_restore(path, self.restore_availability)
    ...
    fn font_from_settings(editor: &settings::EditorSettings) -> Result<FontKey, EditorRenderError> {
        let size = PositiveFinite::new(editor.font_size).ok_or(EditorRenderError::Domain)?;
        let scale = PositiveFinite::new(editor.font_scale).ok_or(EditorRenderError::Domain)?;
        let tabs = NonZeroU32::new(editor.tab_columns).ok_or(EditorRenderError::Domain)?;
        Ok(FontKey::new(editor.font_family, size, scale, tabs))
    }

    fn resolved_font(&self) -> Result<FontKey, EditorRenderError> {
        Self::font_from_settings(&self.settings.active().editor)
    }

    #[cfg(test)]
    fn font() -> Result<FontKey, EditorRenderError> {
    ...
    fn scene(&mut self, revision: SceneRevision, viewport: Size) -> Scene {
        ...
        match self.try_scene(revision, viewport) {
            Ok(scene) => {
                ...
                self.record_profile(
                    EditorSignpostStage::FrameBuildComplete,
                    revision,
                    [
            ...
            Err(error) => {
                ...
                self.record_profile(EditorSignpostStage::FrameBuildFailed, revision, [1, 0, 0]);
    ...
    fn try_scene(
        ...
        let language_identity = self.language_identity();
        let mut diagnostic_markers = 0_usize;
        if self.workspace.is_some() && self.file_tree.is_visible() {
            ...
            for row in rows {
                ...
                if row.selected {
                    let row_origin = Point::new(0.0, top).ok_or(EditorRenderError::Domain)?;
            ...
            .any(|(index, _)| *index == self.tabs.active_index())
        {
            let active_left = sidebar_width + usize_as_f32(self.tabs.active_index()) * TAB_WIDTH
                - self.tab_scroll_x;
            let active_origin = Point::new(active_left, 0.0).ok_or(EditorRenderError::Domain)?;
            let active_size =
        ...
        for (pane_index, pane) in pane_layout.iter().enumerate() {
            let pane_clip = pane_clips[pane_index].ok_or(EditorRenderError::Domain)?;
        ...
        self.record_profile(
            EditorSignpostStage::VisibleLayoutComplete,
            revision,
        ...
        if let (Some(projection), Some(composition)) = (&projection, &self.composition) {
            ...
            for rendered in &rendered_lines {
                ...
                for (left, right) in composition::visual_spans(
                    ...
                    .ok_or(EditorRenderError::Domain)?;
                    let size =
                        Size::new((right - left).max(1.0), 1.0).ok_or(EditorRenderError::Domain)?;
        ...
        let status_background = if let Some(status) = status {
            ...
            let origin = Point::new(editor_origin_x, top).ok_or(EditorRenderError::Domain)?;
            let size =
                Size::new(content_size.width(), LINE_HEIGHT).ok_or(EditorRenderError::Domain)?;
            ...
            .completion_visible_range(language_identity)
        {
            ...
            let overlay_origin = Point::new(left, top).ok_or(EditorRenderError::Domain)?;
            let overlay_size =
                Size::new(width, height.max(1.0)).ok_or(EditorRenderError::Domain)?;
            ...
            for (visible_row, index) in rows.enumerate() {
                ...
                if selected {
                    ...
                    let row_size =
                        Size::new(width, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?;
                    let highlight =
            ...
            .navigation_visible_range(language_identity)
        {
            ...
            for (visible_row, index) in rows.enumerate() {
                ...
                let row_top = top + usize_as_f32(visible_row) * LINE_HEIGHT;
                if row.selected {
                    let origin = Point::new(left, row_top).ok_or(EditorRenderError::Domain)?;
                    let size = Size::new(width, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?;
        ...
        if self.quick_open.is_open() {
            ...
            for (row, (path, selected)) in rows.iter().enumerate() {
                ...
                if *selected {
                    let row_origin = Point::new(left, row_top).ok_or(EditorRenderError::Domain)?;
                    let row_size =
                        Size::new(width, QUICK_OPEN_ROW_HEIGHT).ok_or(EditorRenderError::Domain)?;
                    let selected_quad =
    ...
    fn language_overlay_bounds(
        ...
        let left = (editor_origin_x + CONTENT_INSET)
            .min((editor_origin_x + content_size.width() - width).max(editor_origin_x));
        let origin = Point::new(left, top).ok_or(EditorRenderError::Domain)?;
        let size = Size::new(width, height.max(1.0)).ok_or(EditorRenderError::Domain)?;
    ...
    fn paint_selection(
        ...
        {
            u32::try_from(content.encode_utf16().count()).map_err(|_| EditorRenderError::Domain)?
        } else {
        ...
        let origin = Point::new(start_x, top).ok_or(EditorRenderError::Domain)?;
        let size = Size::new(width, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?;
    ...
    fn paint_overlay_field(
    ...
    ) -> Result<Vec<PendingGlyph>, EditorRenderError> {
        ...
        if overlay_field::Owner::active(self) == Some(owner) {
            ...
            let bounds = Rect::new(
                Point::new(caret_x, view.top).ok_or(EditorRenderError::Domain)?,
                Size::new(CARET_WIDTH, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?,
            );
            builder.push_quad(Quad::new(bounds, caret_color).clipped(clip))?;
        }
        self.collect_glyphs(
    ...
    fn collect_glyphs(
        ...
        for glyph in layout.glyphs() {
            ...
            let font = FontKey::new(
                family,
                PositiveFinite::new(requested_font.size()).ok_or(EditorRenderError::Domain)?,
                PositiveFinite::new(requested_font.scale()).ok_or(EditorRenderError::Domain)?,
            ...
            .ok_or(EditorRenderError::Domain)?;
            let size = Size::new(width, height).ok_or(EditorRenderError::Domain)?;
    ...
    fn publish_atlas_if_needed(
        ...
        self.record_profile(
            EditorSignpostStage::AtlasPublicationBegin,
            revision,
        ...
        let dimension = NonZeroU32::new(snapshot.dimension()).ok_or(EditorRenderError::Domain)?;
    ...
    fn record_atlas_publication_failure(&self) {
        self.record_profile(
            EditorSignpostStage::AtlasPublicationFailed,
            self.profile_scene_revision,
            [1, 0, 0],
        );
    }
    ...
    fn full_atlas_image(
        ...
        pixels: Box<[u8]>,
    ) -> Result<GlyphAtlasImage, EditorRenderError> {
    ...
    fn caret_bounds(
            ...
            (&projection, &self.composition)
        {
            ...
            Some((
                line.display,
                u32::try_from(index - line.base_utf16).map_err(|_| EditorRenderError::Domain)?,
            ))
        } else {
            None
        };
        ...
        let origin = Point::new(x, rendered.top).ok_or(EditorRenderError::Domain)?;
        let size = Size::new(CARET_WIDTH, LINE_HEIGHT).ok_or(EditorRenderError::Domain)?;
    ...
    fn begin_clipboard_operation(&mut self, operation: ClipboardOperation) -> EditorTransition {
        self.pending_field_clipboard = None;
        self.pending_cut = None;
        let mut effect = self.clear_clipboard_status();
        let selection = self.selection;
        if selection.range().is_empty() {
            return EditorTransition::effect(effect);
        }
    ...
    fn handle_clipboard_completion(&mut self, event: &ClipboardEvent) -> EditorTransition {
        ...
        {
            return EditorTransition::effect(self.record_clipboard_protocol_failure(
    ...
    fn handle_close_request(&mut self) -> EditorTransition {
    ...
    fn handle_key(&mut self, physical_key: u16, modifiers: Modifiers) -> EventEffect {
        ...
        if action == Some(KeyAction::Command(EditorCommand::OpenProjectSearch)) {
            return self.open_project_search();
        }
        if self.project_search.is_open() {
            return self.handle_project_search_key(physical_key, command);
        }
        if action == Some(KeyAction::Command(EditorCommand::ToggleFileTree)) {
        ...
        if let Some(KeyAction::Command(
            command @ (EditorCommand::OpenFind | EditorCommand::OpenReplace),
        )) = action
        {
    ...
    fn dispatch_command(&mut self, command: EditorCommand) -> EventEffect {
        match command {
            ...
            EditorCommand::PreviewRustRename => self.open_rust_rename(),
            EditorCommand::PreviewRustFormatting => self.trigger_rust_formatting(),
            EditorCommand::OpenQuickOpen if self.workspace.is_none() => {
                self.record_quick_open_error(&QuickOpenError::NoWorkspace)
            }
            EditorCommand::OpenQuickOpen => match self.quick_open.open(1) {
                Ok(changed) => changed.then(EventEffect::visual).unwrap_or_default(),
                Err(error) => self.record_quick_open_error(&error),
            },
            EditorCommand::OpenProjectSearch => self.open_project_search(),
            EditorCommand::ToggleFileTree if self.workspace.is_none() => {
                self.record_file_tree_error(&FileTreeError::NoWorkspace)
            }
            EditorCommand::ToggleFileTree
                if self.file_tree.is_visible() && self.file_tree.is_focused() =>
            {
                self.file_tree
                    .hide()
            ...
            EditorCommand::ToggleFileTree => match self.file_tree.activate(1) {
                Ok(changed) => changed.then(EventEffect::visual).unwrap_or_default(),
                Err(error) => self.record_file_tree_error(&error),
            },
            EditorCommand::SplitRight => self.split_active_pane(SplitAxis::Columns),
            EditorCommand::SplitDown => self.split_active_pane(SplitAxis::Rows),
    ...
    fn open_project_search_selection(&mut self) -> Result<EventEffect, WorkspaceSelectionError> {
        ...
        let effect = if let Some(tab) = self.tabs.index_for_path(&path) {
            ...
            let document = EditorDocument::open(&path).map_err(WorkspaceSelectionError::File)?;
            project_search::verify_snapshot_match(&document.buffer().snapshot(), &selected)
                .map_err(WorkspaceSelectionError::ProjectSearch)?;
    ...
    fn validate_navigation_document(
        ...
        let change = match document {
            EditorDocument::File { editor, .. } => editor
                .external_change()
                .map_err(WorkspaceSelectionError::File)?,
            EditorDocument::Recovered { conflict, .. }
            | EditorDocument::Unavailable { conflict, .. } => *conflict,
            EditorDocument::Scratch { .. } => {
    ...
    fn active_rust_document(&self) -> Option<RustDocumentInput> {
        ...
        Some(RustDocumentInput::new(
            path,
            workspace_root,
            self.language_identity(),
            self.buffer().snapshot(),
        ))
    }

    fn sync_rust_diagnostics(
        &mut self,
        ...
        let effect = self.rust_diagnostics.sync(input, move |wake| {
            ...
            Arc::new(move || {
                ...
                let admission = producer.submit(EditorWorkerOutput::Language(wake), 0);
                #[cfg(all(alpine_native_validation, not(test)))]
    ...
    fn synchronize_language_after_event(
        &mut self,
        context: &mut AppContext<'_, EditorWorkerOutput>,
    ) -> bool {
        ...
        if let Some(continuation) = pending.continuation {
            ...
            let admission = context
                .external_producer()
                .submit(EditorWorkerOutput::Language(continuation), 0);
            #[cfg(all(alpine_native_validation, not(test)))]
            record_native_validation_language_submission(admission);
            #[cfg(any(not(alpine_native_validation), test))]
            let _ = admission;
    ...
    fn finish_event(
        ...
        match self.prepare_find_request() {
            Ok(Some(request)) => {
                ...
                match context.spawn(move || EditorWorkerOutput::Find(request.execute())) {
                    Ok(_) => {}
                    Err(error) => {
...
impl AppDelegate for EditorApp {
    type WorkerOutput = EditorWorkerOutput;

    fn event(&mut self, event: &SurfaceEvent, context: &mut AppContext<'_, EditorWorkerOutput>) {
        #[cfg(all(alpine_native_validation, not(test)))]
        ...
        self.record_profile(
            EditorSignpostStage::EventDispatchBegin,
            SceneRevision::new(0),
    ...
    fn worker_result(
        ...
        result: EditorWorkerOutput,
        context: &mut AppContext<'_, EditorWorkerOutput>,
    ) {
        let language_result = matches!(&result, EditorWorkerOutput::Language(_));
        #[cfg(all(alpine_native_validation, not(test)))]
        let mut language_visual_changed = false;
        let mut effect = match result {
            EditorWorkerOutput::Find(result) => self.apply_find_output(result),
            EditorWorkerOutput::QuickOpen(result) => self.apply_quick_open_output(result),
            EditorWorkerOutput::ProjectSearch(result) => self.apply_project_search_output(result),
            EditorWorkerOutput::FileTree(result) => self.apply_file_tree_output(result),
            EditorWorkerOutput::Settings(result) => self.apply_settings_output(*result),
            EditorWorkerOutput::Language(wake) => {
                #[cfg(all(alpine_native_validation, not(test)))]
                NATIVE_VALIDATION_LANGUAGE_FOREGROUND_RESULTS
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            ...
            EditorWorkerOutput::WorkspaceEdit(output) => self.apply_workspace_edit_output(output),
            EditorWorkerOutput::WorkspaceEditPublication(output) => {
...
impl EditorApp {
    ...
    fn submit_settings_request(
        ...
        let result = context
            .spawn(move || EditorWorkerOutput::Settings(Box::new(request.execute())))
            .map(|_| ());
    ...
    fn submit_workspace_edit_preparation(
        ...
        if force_workspace_edit_preparation_submission_failure!(self)
            || context
                .spawn(move || EditorWorkerOutput::WorkspaceEdit(request.execute()))
                .is_err()
        {
    ...
    fn submit_workspace_edit_publication(
        ...
        if force_workspace_edit_publication_submission_failure!(self)
            || context
                .spawn(move || EditorWorkerOutput::WorkspaceEditPublication(request.execute()))
                .is_err()
        {
    ...
    fn submit_file_tree_request(&mut self, context: &mut AppContext<'_, EditorWorkerOutput>) {
        match self.prepare_file_tree_request() {
            Ok(Some(request)) => {
                let identity = request.identity();
                let failed = force_file_tree_submission_failure!(self)
                    || context
                        .spawn(move || EditorWorkerOutput::FileTree(request.execute()))
                        .is_err();
    ...
    fn submit_quick_open_request(&mut self, context: &mut AppContext<'_, EditorWorkerOutput>) {
        match self.prepare_quick_open_request() {
            Ok(Some(request)) => {
                ...
                let submission_failed = force_submission_failure
                    || context
                        .spawn(move || EditorWorkerOutput::QuickOpen(request.execute()))
                        .is_err();
    ...
    fn submit_project_search_request(&mut self, context: &mut AppContext<'_, EditorWorkerOutput>) {
        match self.prepare_project_search_request() {
            Ok(Some(request)) => {
                let identity = request.identity();
                let failed = force_project_search_submission_failure!(self)
                    || context
                        .spawn(move || EditorWorkerOutput::ProjectSearch(request.execute()))
                        .is_err();
...
fn editor_clipboard_shortcut(event: &SurfaceEvent) -> Option<ClipboardOperation> {
...
fn local_utf16(
    ...
    line_start: usize,
    offset: usize,
) -> Result<u32, EditorRenderError> {
    let prefix = snapshot.slice(line_start..offset)?;
    u32::try_from(prefix.encode_utf16().count()).map_err(|_| EditorRenderError::Domain)
}
...
fn diagnostic_underline_bounds(
    ...
    let origin =
        Point::new(start_x, baseline + layout.descent() + 1.0).ok_or(EditorRenderError::Domain)?;
    let size = Size::new((end_x - start_x).max(1.0), 1.0).ok_or(EditorRenderError::Domain)?;
    Ok(Rect::new(origin, size))
}
...
/// Non-shipping native Alpine Editor process qualification.
...
pub mod native_validation {
    ...
    use super::{
        CONTENT_INSET, DEFAULT_SCALE, EditorApp, EditorError, FONT_FAMILY, KEY_A, KEY_DOWN, KEY_E,
        KEY_F, KEY_P, KEY_RETURN, KEY_S, KEY_UP, TREE_ROW_HEIGHT, WINDOW_HEIGHT, WINDOW_WIDTH,
        Workspace, native_explicit_path_app_with_session, native_validation_dispatch_counts,
        recovery, recovery_editor_error,
    };
    ...
    fn resolve_recovery_through_save(app: &mut EditorApp) -> Result<(), SurfaceError> {
        let request = app
            .capture_recovery_request()
    ...
    pub fn qualify_retained_recovery_journal(
        ...
        expected: &Path,
        workspace: bool,
    ) -> Result<RetainedRecoveryEvidence, EditorError> {
        let state = recovery::load(journal)
            .map_err(|error| recovery_editor_error(expected, journal, error))?;
    ...
    pub(super) fn qualify_production_window_application(
        application: Application<EditorApp>,
    ...
    -> Result<NativeProcessEvidence, Box<dyn std::error::Error>> {
        let path = std::env::temp_dir().join(format!(
            "alpine-editor-native-process-{}.txt",
            std::process::id()
        ));
    ...
    pub fn qualify_file_tree_process() -> Result<NativeFileTreeEvidence, Box<dyn std::error::Error>>
    {
        ...
        let root = std::env::temp_dir().join(format!(
            "alpine-editor-native-tree-{}-{nonce}",
            std::process::id()
        ));
        fs::create_dir(&root)?;
    ...
    -> Result<NativeProjectSearchEvidence, Box<dyn std::error::Error>> {
        let nonce = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
        let root = std::env::temp_dir().join(format!(
            "alpine-editor-native-project-search-{}-{nonce}",
            std::process::id()
        ));
        fs::create_dir(&root)?;
    ...
    fn qualify_file_tree_path(
        ...
        let delegate = EditorApp::from_workspace(text_system, workspace)?;
        let clear = alpine_core::LinearRgba::new(0.02, 0.02, 0.02, 1.0).ok_or(
            EditorError::Runtime(alpine_runtime::RuntimeError::Surface(
                alpine_platform_macos::SurfaceError::invariant(
                    alpine_platform_macos::SurfaceOperation::Application,
                ),
            )),
        ...
        let descriptor = SurfaceDescriptor::new(
            "Alpine Editor native file tree",
            f64::from(WINDOW_WIDTH),
            f64::from(WINDOW_HEIGHT),
            f64::from(DEFAULT_SCALE),
        )?;
    ...
    fn qualify_project_search_path(
        ...
        let descriptor = SurfaceDescriptor::new(
            "Alpine Editor native project search",
            f64::from(WINDOW_WIDTH),
            f64::from(WINDOW_HEIGHT),
            f64::from(DEFAULT_SCALE),
        )?;
    ...
    fn event_handler(
        state: &Rc<RefCell<Application<EditorApp>>>,
...
mod rust_diagnostics_scene_tests;
...
mod session_integration_tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut restored = EditorApp::from_session(tests::TestTextSystem, persisted)
            .map_err(|error| error.to_string())?;
        ...
        let mut switched = EditorApp::from_session(tests::TestTextSystem, switched_state.clone())
            .map_err(|error| error.to_string())?;
        ...
        let mut clamped = EditorApp::from_session(tests::TestTextSystem, clamped_state)
            .map_err(|error| error.to_string())?;
        ...
        let bounded_view = EditorApp::clamp_view_to_document(
            &EditorDocument::scratch("🦀abcd\nx\ny\nz"),
            DocumentViewState {
        ...
        let mut dirty = EditorApp::new(tests::TestTextSystem)?;
        if let EditorDocument::Scratch {
            buffer,
            clean_revision,
            ..
        } = &mut dirty.document
        ...
        let mut defaulted =
            with_session_path(EditorApp::new(tests::TestTextSystem)?, Ok(expected.clone()));
    ...
    fn session_restore_error_messages_and_rejections_are_structured() {
        ...
        assert!(matches!(
            EditorApp::from_session(tests::TestTextSystem, invalid),
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let root = std::env::temp_dir().join(format!(
            "alpine-editor-recovery-branches-{}-{}",
            std::process::id(),
            SESSION_TEST_SEQUENCE.fetch_add(1, Ordering::Relaxed)
        ));
        fs::create_dir_all(&root)?;
        ...
        let mut scratch = EditorDocument::recover(None, &recovered)?;
        ...
        let mut conflicted = EditorDocument::recover(Some(&invalid), &recovered)?;
        ...
        let mut unavailable =
            EditorDocument::open_for_restore(&invalid, RestoreAvailability::AllowPlaceholder)?;
        ...
        assert_eq!(
            EditorApp::index_recoveries(1, vec![recovered.clone(), recovered]),
            Err(SessionRestoreError::Invalid)
        );

        let capture_path = root.join("capture").join("session.bin");
        let mut invalid_configuration = EditorApp::new(tests::TestTextSystem)?;
        ...
        let mut capture = EditorApp::new(tests::TestTextSystem)?;
        capture
        ...
        let mut degraded = EditorApp::new(tests::TestTextSystem)?;
        degraded
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut app = EditorApp::open_file(tests::TestTextSystem, &document_path)?;
        ...
        let equal_text = EditorApp::from_recovery(tests::TestTextSystem, equal_text)
            .map_err(|error| error.to_string())?;
        assert_eq!(equal_text.buffer().snapshot().text(), "base");
        assert!(equal_text.document.is_dirty());

        let mut unchanged = EditorApp::from_recovery(tests::TestTextSystem, recovered.clone())
            .map_err(|error| error.to_string())?;
        ...
        let mut modified = EditorApp::from_recovery(tests::TestTextSystem, recovered.clone())
            .map_err(|error| error.to_string())?;
        ...
        let mut deleted = EditorApp::from_recovery(tests::TestTextSystem, recovered)
            .map_err(|error| error.to_string())?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let root = std::env::temp_dir().join(format!(
            "alpine-editor-partial-recovery-{}-{}",
            std::process::id(),
            SESSION_TEST_SEQUENCE.fetch_add(1, Ordering::Relaxed)
        ));
        fs::create_dir_all(&root)?;
        ...
        assert!(matches!(
            EditorApp::from_session(tests::TestTextSystem, state.clone()),
        ...
        let mut clean = EditorApp::new(tests::TestTextSystem)?;
        clean
