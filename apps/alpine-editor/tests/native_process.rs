...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if std::env::var_os("ALPINE_EDITOR_NATIVE_LSP_SERVER").is_some() {
        lsp_mock_server::main();
        return Ok(());
    }
    if std::env::var_os("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_CHILD").is_some() {
        let omitted = std::env::var("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_OMIT").ok();
        let result = alpine_editor::native_validation::qualify_editor_accessibility_process();
        if let Some(omitted) = omitted {
            return match result {
                Err(error) => {
                    let error = error.to_string();
                    alpine_editor::native_validation::validate_native_accessibility_omission_failure(
                        &omitted,
                        &error,
                    )
    ...
    if std::env::var_os("ALPINE_EDITOR_NATIVE_PROCESS_SCOPE").as_deref()
        == Some(std::ffi::OsStr::new("accessibility"))
    {
        qualify_accessibility_child()?;
        return Ok(());
    }
    let initial = alpine_editor::initial_scene()?;
    ...
    qualify_shipping_executable()?;
    let evidence = alpine_editor::native_validation::qualify_clipboard_and_close_process()?;
    ...
    let tree = alpine_editor::native_validation::qualify_file_tree_process()?;
    ...
    let search = alpine_editor::native_validation::qualify_project_search_process()?;
...
fn qualify_accessibility_child() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = std::env::temp_dir().join(format!(
        "alpine-editor-native-accessibility-child-{}-{nonce}",
        std::process::id()
    ));
    ...
    std::fs::write(
        &server,
        "#!/bin/sh\nprintf 'wrapper-invoked:%s\\n' \"$$\" >> \"$ALPINE_EDITOR_NATIVE_LSP_TRACE\"\nexport ALPINE_EDITOR_NATIVE_LSP_SERVER=1\nexec \"$ALPINE_EDITOR_NATIVE_PROCESS_EXE\"\n",
    )?;
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        let run_child = |omitted: Option<&str>, home: &std::path::Path| {
            ...
            command
                .env("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_CHILD", "1")
                .env("ALPINE_EDITOR_NATIVE_PROCESS_EXE", &executable)
                .env("ALPINE_RUST_ANALYZER", &server)
                .env("ALPINE_EDITOR_NATIVE_LSP_TRACE", &language_trace)
                .env("HOME", home)
                .env_remove("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_OMIT")
                .stdout(Stdio::piped())
                .stderr(Stdio::piped());
            if let Some(omitted) = omitted {
                command.env("ALPINE_EDITOR_NATIVE_ACCESSIBILITY_OMIT", omitted);
            }
        ...
        let first_trace_complete =
            alpine_editor::native_validation::validate_native_language_startup_trace(&first_trace)
                .is_ok();
        let (status, stdout, stderr, trace) =
            if alpine_editor::native_validation::hosted_terminal_stall_retry_allowed(
                &evidence_mode,
                0,
...
fn require_omission_output(stdout: &str, omitted: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    alpine_editor::native_validation::validate_native_accessibility_omission_failure(omitted, error)
...
fn require_language_trace_prefix(
    ...
    alpine_editor::native_validation::validate_native_language_startup_prefix(trace).map_err(
...
fn require_language_startup_trace(trace: &str) -> Result<(), Box<dyn std::error::Error>> {
    alpine_editor::native_validation::validate_native_language_startup_trace(trace).map_err(
...
fn qualify_shipping_executable() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let nonce = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let root = std::env::temp_dir().join(format!(
        "alpine-editor-shipping-process-{}-{nonce}",
        std::process::id()
    ));
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        let mut child = Command::new(env!("CARGO_BIN_EXE_alpine-editor"))
            .arg(&path)
            .env(
                "ALPINE_EDITOR_NATIVE_PROCESS_SCENARIO",
                "production-single-window",
            )
            .env("ALPINE_EDITOR_DOGFOOD_OUTPUT", &diagnostic)
            .env("ALPINE_EDITOR_DOGFOOD_WORKLOAD_ID", "hosted-close")
            .env("ALPINE_EDITOR_DOGFOOD_REVISION", "a".repeat(40))
            .env(
                "ALPINE_EDITOR_DOGFOOD_CAPTURED_AT_UTC",
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                let status = child.wait()?;
                return Err(format!(
                    "shipping Alpine Editor exceeded {timeout:?} and was terminated with {status}"
                )
                .into());
            }
            thread::sleep(Duration::from_millis(10));
        ...
        if !status.success() {
            return Err(format!(
                "shipping Alpine Editor failed with {status}; stdout={stdout:?}; stderr={stderr:?}"
            )
            .into());
        }
        ...
        assert_eq!(
            ...
            Some("alpine-editor-internal-diagnostic/v1")
        );
        assert_eq!(
            diagnostic
...
fn qualify_recovery_launch_processes(
    ...
    let file_journal = alpine_editor::native_validation::retain_pending_recovery_fixture(
        &file_home,
        &recovered_file,
        "unsaved recovery\n",
    )?;
    ...
    let file_recovery = alpine_editor::native_validation::qualify_retained_recovery_journal(
        &file_journal,
        &requested_file,
        false,
    )?;
    ...
    let folder_journal = alpine_editor::native_validation::retain_pending_recovery_fixture(
        &folder_home,
    ...
    let folder_recovery = alpine_editor::native_validation::qualify_retained_recovery_journal(
        &folder_journal,
        &requested_folder,
        true,
    )?;
...
fn run_recovery_launch_process(
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_alpine-editor"))
        .arg(requested)
        .env("ALPINE_EDITOR_NATIVE_PROCESS_SCENARIO", scenario)
        .env("ALPINE_EDITOR_NATIVE_EXPECTED_PATH", requested)
        .env("HOME", home)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()?;
