...
jq -n '
    ...
    [
        ...
        record("Native Display Link Target Latency"; 196; 0; 0; 0; 70; 0; 0),
        record("Native Target Presentation Latency"; 197; 0; 0; 0; 80; 0; 0),
        record("Native Actual Presentation Latency"; 198; 0; 0; 0; 90; 0; 0),
        record("Native Presentation Callback Lag"; 199; 0; 0; 0; 10; 0; 0)
    ]
' > "$valid"

output=$scratch/output
valid_stderr=$scratch/valid.stderr
if ! scripts/analyze-studio-profile-v2.sh "$valid" "$output" 125 3 \
...
grep -Fq 'schema=alpine-studio-profile-analysis/v2' "$output/report.txt"
grep -Fq 'record_count=21' "$output/report.txt"
grep -Fq 'presented_sample_count=1' "$output/report.txt"
grep -Fq 'actual_presentation_sample_count=1' "$output/report.txt"
...
awk -F '\t' '$1 == "native_actual_presentation" && $2 == 1 && $4 == 90 {found=1} END {exit !found}' "$output/summary.tsv"
awk -F '\t' '$1 == "native_presentation_callback_lag" && $2 == 1 && $4 == 10 {found=1} END {exit !found}' "$output/summary.tsv"
...
expect_failure identity 'process identity changed' \
    scripts/analyze-studio-profile-v2.sh "$scratch/identity.json" "$scratch/identity-output" 125 3

make_invalid order '.[1].machTimestamp = 99'
expect_failure order 'machTimestamp decreased' \
    scripts/analyze-studio-profile-v2.sh "$scratch/order.json" "$scratch/order-output" 125 3
...
expect_failure correlation 'nonzero event does not match correlation' \
    scripts/analyze-studio-profile-v2.sh "$scratch/correlation.json" "$scratch/correlation-output" 125 3
...
expect_failure native-revision 'native stage carries nonzero revision identity' \
    scripts/analyze-studio-profile-v2.sh "$scratch/native-revision.json" "$scratch/native-revision-output" 125 3

make_invalid duplicate '. + [(. [0] | .machTimestamp = 200)]'
expect_failure duplicate 'duplicate stage for one correlation' \
    scripts/analyze-studio-profile-v2.sh "$scratch/duplicate.json" "$scratch/duplicate-output" 125 3
...
expect_failure unknown 'unknown profile stage' \
    scripts/analyze-studio-profile-v2.sh "$scratch/unknown.json" "$scratch/unknown-output" 125 3

make_invalid grammar '.[3].eventMessage += " trailing"'
expect_failure grammar 'eventMessage does not match' \
    scripts/analyze-studio-profile-v2.sh "$scratch/grammar.json" "$scratch/grammar-output" 125 3

make_invalid format '.[3].formatString = "wrong"'
expect_failure format 'formatString does not match' \
    scripts/analyze-studio-profile-v2.sh "$scratch/format.json" "$scratch/format-output" 125 3

expect_failure timebase 'timebase numerator must be a positive integer' \
    scripts/analyze-studio-profile-v2.sh "$valid" "$scratch/timebase-output" 0 3
expect_failure existing-output 'output already exists' \
    scripts/analyze-studio-profile-v2.sh "$valid" "$output" 125 3

printf 'Studio profile v2 analyzer tests passed\n'
