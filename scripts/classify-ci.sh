...
if matches '^(\.github/workflows/(ci|nightly-assurance)\.yml$|assurance/miri-studio-partitions\.tsv$|scripts/(classify-ci|test-classifier|run-miri-partition|test-miri-partitions)\.sh$)'; then
    coverage=true
    mutation=true
    kani=true
    miri=true
