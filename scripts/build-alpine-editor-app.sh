...
bundle="$staging_dir/Alpine Editor.app"
contents="$bundle/Contents"
macos="$contents/MacOS"
resources="$contents/Resources"
mkdir -p "$macos" "$resources"
cp "$executable" "$macos/alpine-editor"
chmod 0755 "$macos/alpine-editor"
if ! cmp -s "$executable" "$macos/alpine-editor"; then
...
binary_sha256=$(shasum -a 256 "$macos/alpine-editor" | awk '{print $1}')
binary_bytes=$(wc -c < "$macos/alpine-editor" | tr -d ' ')
...
bundle_identifier = "com.dbuddha.alpine-editor"
...
backup="$output_parent/.Alpine Editor.app.previous.$$"
if [ -e "$backup" ]; then
