...
usage() {
    cat <<'EOF'
usage: scripts/analyze-studio-profile-v2.sh RECORDS_JSON OUTPUT_DIR TIMEBASE_NUMER TIMEBASE_DENOM
...
awk -F '\t' -v numer="$numer" -v denom="$denom" '
    ...
    function native_metric(stage) {
        ...
        if (stage == "Native Display Link Target Latency") return "native_display_link_target"
        if (stage == "Native Target Presentation Latency") return "native_target_presentation"
        if (stage == "Native Actual Presentation Latency") return "native_actual_presentation"
        if (stage == "Native Presentation Callback Lag") return "native_presentation_callback_lag"
        return ""
    }
    {
        key = "x" $7
...
awk -F '\t' '
    ...
    END {
        split("Frame Build Begin|Visible Layout Begin|Visible Layout Complete|Text Summary|Layout Cache Summary|Glyph Atlas Summary|Frame Build Complete|Native Event Handler Latency|Native Frame Queue Latency|Native Submission Latency|Native GPU Terminal Observed Latency|Native Presented Handler Latency|Native Terminal Record Latency|Native Display Link Target Latency|Native Target Presentation Latency|Native Actual Presentation Latency|Native Presentation Callback Lag", required, "|")
        for (key in visual)
...
actual_presentation_samples=$(awk -F '\t' '$1 == "native_actual_presentation" {count++} END {print count + 0}' "$raw_samples")
omission_count=$(awk 'NR > 1 {count++} END {print count + 0}' "$staging/omissions.tsv")

cat > "$staging/report.txt" <<EOF
schema=alpine-studio-profile-analysis/v2
...
actual_presentation_sample_count=$actual_presentation_samples
